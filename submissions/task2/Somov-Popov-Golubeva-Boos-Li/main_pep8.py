...
p = figure(title='Data', x_axis_label='date', y_axis_label='value',
...
d = figure(title='Data', x_axis_label='date',
           x_axis_type='datetime', width=1000)
...
def arima(train, order, test):
    ar = figure(title='Data', x_axis_label='date',
                x_axis_type='datetime', width=1000)

    model = sm.tsa.ARIMA(training_data.Value, order=order, freq='MS').fit()
    y_r = model.predict(
        start=training_data.shape[0], end=training_data.shape[0] + testing_data.shape[0]-1)
